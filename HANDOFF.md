...
Bambu-style filament feed assistance driven by the autoloader, following the
**active toolhead**. Not yet specced. Open questions to settle before building:

- What is the trigger — encoder slip on the toolhead extruder, a physical
  buffer switch, or purely inferred from commanded vs measured extrusion?
- The drive motor is shared across all six paths, and the selector must be
  parked at the active path to assist it. That conflicts with any other
  autoloader activity during a print, and with tool changes, which move the
  active path constantly. **This is the hard part** — a toolchanger changes
  which path needs assistance mid-print, and the selector has to keep up.
- Does assist run continuously during a print, or only on demand?
- Hardware: is a buffer being added, or is this software-only using the
  existing per-path encoders?
