...
| Active tool has three disagreeing sources | **Root cause found (2026-09-03): `detection_pin` is commented out on all six `[tool]` sections** (`~/printer_data/config/Toolchanger/Tool-Heads/T*.cfg`), so `toolchanger.has_detection` is False and the toolchanger structurally cannot sense what is mounted — it only remembers what it last did, and a restart erases that. Measured live: `toolchanger.status=uninitialized`, `tool_number=-1`, `detected_tool_number=-1`, while `tool_probe_endstop.active_tool_number=1` and `toolhead.extruder=extruder` (T0). **This is worse than a display bug.** `initialize_on: first-use` means the next `T<n>` self-initialises from `detected_tool`, which is None, so `active_tool` becomes None and the change proceeds to *pick up* a tool — while that tool is already on the carriage. `require_tool_present` is unset (default False), so nothing stops it. Every autoloader sequence issues `T<n>` via `_switch_tool`. Remedy today is `INITIALIZE_TOOLCHANGER TOOL=<mounted>`; nothing prompts for it. Real fix is to enable the detection pins (note the commented lines in T4.cfg and T5.cfg both say `et4:PB6` — one is a copy-paste error) or to seed the toolchanger from `tool_probe_endstop` at startup, plus a guard that refuses to issue `T<n>` while `status == uninitialized`. |
