...
| Active tool disagrees before initialization | **Corrected 2026-09-03 — this was filed, and then twice re-filed, as more dangerous than it is. There is no crash.** StealthChanger's `initialize_gcode` is already `DETECT_ACTIVE_TOOL_PROBE` + `_INITIALIZE_FROM_DETECTED_TOOL`, which calls `INITIALIZE_TOOLCHANGER T={tool_probe_endstop.active_tool_number}`. That re-enters `initialize()` while status is `INITIALIZING`, so it skips re-running the gcode but does run `_configure_toolhead_for_tool`, setting `active_tool` from the probe. `initialize_on: first-use` therefore makes the first `T<n>` self-initialise *correctly*, after which `select_tool` finds `active_tool == tool` and returns without moving. If the probe cannot tell (`-1`), `_INITIALIZE_FROM_DETECTED_TOOL` responds error and PAUSEs — a safe stop. **Verified live:** bare `INITIALIZE_TOOLCHANGER` took `uninitialized / tool_number -1 / active_tool None / toolhead.extruder extruder` to `ready / 1 / tool T1 / extruder1` with no motion. G28 (`homing_override`) and `PRINT_START` both call it, so normal operation is never in this state. **What is actually left:** in the window between a Klipper restart and the first initialize, `toolhead.extruder` still points at T0 while another tool is mounted, so a bare `G1 E…` typed in that window targets the wrong extruder (usually blocked by `min_extrude_temp`, not always). And any UI reading `toolhead.extruder` — the Mainsail panel, `AutoloaderPanel.vue:1693` — shows T0 in that window while the LEDs, reading the probe, show the truth. Worth fixing as a display honesty issue: surface `toolchanger.status == uninitialized` rather than confidently naming a tool. Not worth a motion guard; the toolchanger already has one. |
