...
| Active tool has three disagreeing sources | **Root cause (2026-09-03): the toolchanger has no detection at all.** `detection_pin` is commented out on all six `[tool]` sections, so `toolchanger.has_detection` is False — it only remembers what it last did, and a restart erases that. Measured live: `toolchanger.status=uninitialized`, `tool_number=-1`, `detected_tool_number=-1`, while `tool_probe_endstop.active_tool_number=1` and `toolhead.extruder=extruder` (T0). **Why the pins are off:** `detection_pin` and `[tool_probe TN] pin:` are both `etN:PB6` — the same physical pin, already owned by Tap. Uncommenting gives "pin used multiple times", not a second Z endstop (`detection_pin` registers a *button*, not an endstop), so a free input pin plus a physical dock switch would work — but there is no such switch on this machine and one is not needed, see below. **Severity is higher than a display bug.** `initialize_on: first-use` means the next `T<n>` self-initialises from `detected_tool`, which is None, so `active_tool` becomes None and the change proceeds to *pick up* a tool already on the carriage. `require_tool_present` is unset (default False), so nothing stops it. Every autoloader sequence issues `T<n>` via `_switch_tool`. **Fix, needing no pin:** `tool_probe_endstop` already knows — its `_detect_active_tool()` runs at `klippy:connect` and finds the mounted tool as the one probe *not* triggered, which is why it reads 1 correctly right now. So seed the toolchanger from it at startup (`INITIALIZE_TOOLCHANGER T=<n>` — note `T=` for the number; `TOOL=` expects a tool *name*), and guard `_switch_tool` to refuse while `status == uninitialized` rather than letting the self-init guess. Manual remedy today is that same `INITIALIZE_TOOLCHANGER T=<mounted>`; nothing currently prompts for it. |
