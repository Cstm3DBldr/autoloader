...
| `docs/RUNOUT_MIDPRINT.md` | **Design note, nothing built.** What should happen when a roll runs out *during* a print: flag the path `low` rather than wiping it empty, keep printing on the Bowden's worth of tail, and pause before the tail reaches the extruder gears — past that point nothing in the machine can drive it and `RECOVERY.md` is the only way back. Carries the finding that Klipper cannot see `;TYPE:` markers at all (`klippy/gcode.py` strips every comment before dispatch), so pausing in infill has to come from Moonraker reading the file |
