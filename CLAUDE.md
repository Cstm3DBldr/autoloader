...
| `docs/TIPFORM_CAL.md` | **Design note, nothing built.** The last autoloader calibration still done by hand-editing a config: form a tip, let the operator score it, and save the answer **per product line** rather than per path, because the behaviour belongs to the filament and a spool moves between paths. Records why `variables.cfg` is the only safe home for it (`post_update.sh` overwrites the tracked brand files), the four-link resolution chain it adds, and the experiment behind Mike's "3-4 materials": the shipped table applies PLA's measured -15C shear delta to every other row, which assumes deltas transfer across glass transitions. Measuring PLA, PETG and one styrenic tests that |
| `docs/PRINTER.md` | **Project-neutral, written to be copied into another repo verbatim.** The machine itself rather than this project: access routes, reading the console out of Moonraker's `gcode_store` instead of asking for screenshots, which restart reloads what, the MCU layout, and the four edges of two projects sharing one printer — restarting under someone else's run, per-repo `[update_manager]` blocks, `printer.cfg` being the machine's not a project's, and symlinked-vs-copied deciding whether a change actually arrived. Says up front that it is a map and not a snapshot: re-read the machine before trusting any inventory claim in it |
...
| `scripts/can_restart.py` | Answers one question: is it safe to restart Klipper right now? Exit 1 and a reason when a heater is active or a nozzle is still above 60C, or when `idle_timeout` says something is running. **A toolhead MCU shuts ITSELF down when the host disappears while one of its heaters is on** — the EBB36 doing its job — and every connect afterwards says "Can not update MCU 'etN' config as it is shutdown" until a `FIRMWARE_RESTART`. That happened to et4 and then et0 on 2026-09-12, both times from a restart fired straight after a load left a nozzle hot. `klipper_service_restart.sh` calls it; `SA_FORCE=1` overrides |
...
| `scripts/patch_klipperscreen.sh` | Adds the `addons/` hook to KlipperScreen's `screen.py` **on KlipperScreen builds that do not have one**. It was what we proposed upstream, and upstream took it: KlipperScreen#1770 merged 2026-09-13 (`8abe645c`), so this is now a fallback for older installs rather than the normal path. It guards on the `_load_addons` marker, which upstream's hook also has, so it stands aside by itself — and says WHICH reason it skipped for, because those are different news. Guarded: refuses unless the exact anchor matches, idempotent, backs up, byte-compiles and restores on failure, `--revert` undoes it |
| `scripts/ks_addons_state.sh` | Is KlipperScreen's own add-on loader present, and is it switched on? Exit 0 both, 1 present-but-off, 2 no upstream hook (our patch is in charge). **Exists because upstream's hook is gated on `enable_addons` in `[main]`, default False** — review asked for that, on the grounds that it runs code KlipperScreen did not write. So on any printer that updates past the merge, our patch correctly stands aside and `sa_autoloader` silently stops loading. Nothing errors; the touchscreen just stops following a guide opened in Mainsail until a panel has been opened once. `install.sh` ASKS before setting the flag (`SA_KS_ADDONS=yes|no` unattended, and no terminal means no); `post_update.sh` only ever reports it |
...
| `HANDOFF.md` | Shared memory between local and cloud sessions, because local `~/.claude` memory is invisible to a cloud one. Carries the access routes and the commands that actually get used (reading the printer's console straight out of Moonraker's gcode store, the two different service restarts, `scp -O`), the hardware and software topology, where things stand, and the forward-looking backlog that `TODO.md` does not cover — buffer/feed-assist, spool rewind, portability to other StealthChanger builds. **No secrets: every route is key-based or unauthenticated on the LAN.** `TODO.md` is what is broken; this is how the project works and what it is for |
...
| `scripts/patch_klipperscreen.sh` | edits `~/KlipperScreen/screen.py` | **re-applied by post_update.sh on every update.** A KlipperScreen update replaces screen.py and takes the hook with it silently, so this runs every time rather than once — and on a build that has upstream's own hook it detects that and does nothing. `install.sh --uninstall` reverts it, but only when no other add-on is left using it |
| `scripts/ks_addons_state.sh` | read-only; called by install.sh and post_update.sh | not copied — run from the repo |
