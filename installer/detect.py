...
def write_uuid_menu(path, found, assigned):
    """Turn discovered UUIDs into a pickable menu.

    Kconfig has no way to offer a list that is only known at run time, so the
    list is generated as a fragment and pulled in with `osource` -- optional,
    so a printer where nothing was found still parses.
    """
    if not found:
        try:
    ...
    lines = ["# Generated by installer/detect.py. Do not edit; it is rewritten",
             "# on every run and ignored by git.", ""]
    lines.append("choice")
    lines.append('    prompt "Which board is the autoloader?"')
    lines.append("    help")
    lines.append("      These answered a CAN scan, which means they are powered")
    lines.append("      and not yet claimed by any config -- so one of them is")
    lines.append("      almost certainly your new autoloader board.")
    if assigned:
        lines.append("")
        lines.append("      Already in use, for reference:")
        for u, name in sorted(assigned.items()):
            lines.append("        %s  ->  %s" % (u, name))
    lines.append("")
    for i, u in enumerate(found):
        lines.append("config UUID_PICK_%d" % i)
        lines.append('    bool "%s"' % u)
    lines.append("config UUID_PICK_MANUAL")
    lines.append('    bool "Type it in myself"')
    lines.append("endchoice")
    lines.append("")
    for i, u in enumerate(found):
        lines.append("if UUID_PICK_%d" % i)
        lines.append("config MCU_UUID")
        lines.append('    default "%s"' % u)
        lines.append("endif")
    lines.append("")
...
def main():
    ...
    # ── CAN ──────────────────────────────────────────────────────────────────
    uuids, note = detect_can_uuid(files, args.config_dir)
    assigned = detect_assigned_uuids(files)
    scanned = [u for u in uuids if u.lower() not in assigned]
    ...
    # ── write ────────────────────────────────────────────────────────────────
    new = [k for k in proposed if k not in existing]
    if not new:
        print("")
